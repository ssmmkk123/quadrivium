"""Shared optional root solver histories and iteration callbacks.

Policies are context-local, so concurrent tasks can select different retention.
Nested root solvers inherit the memory policy; only the outer history invokes the
...
_policy = ContextVar("quadrivium_root_history", default=None)
...
def monitor(function):
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            result = RootResult(stopped.x, iterations=stopped.history.steps,
                                converged=False, method=function.__name__,
                                history=list(stopped.history), message="stopped by callback")
            # Evaluate the residual at the returned iterate; fixed-point maps
            # use g(x)-x. A callback stop is reported as non-convergence.
            bound = sig.bind_partial(*args, **kwargs).arguments
            objective = bound.get("f", bound.get("F"))
            if callable(objective):
                result.f_root = objective(stopped.x)
            else:
                fixed_map = bound.get("g", bound.get("G"))
                if callable(fixed_map):
                    result.f_root = fixed_map(stopped.x) - stopped.x
            return result
        finally:
            _policy.reset(token)
