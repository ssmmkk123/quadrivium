...
**835 public functions and classes across 13 subpackages. 338 tests, all passing.
Depends only on NumPy.**

## Installation

numethods requires Python 3.9 or newer and depends only on NumPy. From a clone
of this repository, install it with:

```bash
python -m pip install .
```

For an editable development installation with the test dependencies:
...
nm.brent(lambda x: x**3 - 2*x - 5, 1, 3).root      # 2.0945514815423265
nm.quad(lambda x: np.exp(-x*x), -np.inf, np.inf)   # sqrt(pi), to 5e-15
nm.solve_ivp(lambda t, y: -2*y, (0, 1), [1.0]).y[-1, 0]
nm.minimize(rosenbrock, [-1.2, 1.0], method="bfgs")

nm.milstein(drift, diffusion, (0, 1), [1.0])       # SDEs, strong order 1
nm.wavedec(signal, "db4", level=5)                 # wavelets
nm.sylvester(A, B, C)                              # A X + X B = C in O(n^3)
nm.gragg_bulirsch_stoer(f, (0, 10), y0)            # extrapolation
nm.newton_krylov(F, x0, precond=M)                 # Jacobian-free Newton
nm.aaa(f, points)                                  # rational approximation
```
...
takes n = 1000 Bratu from 102,101 residual evaluations to 21.
...
numethods is available under the [MIT License](LICENSE).

Contributions are welcome. See [CONTRIBUTING.md](CONTRIBUTING.md) for the
development workflow and validation expectations. Please report security
